...
function BugPageNumContainer(){
    const {isLoading:isPostNumLoading, data:bugPostNum} = useQuery<number>("postBugNum", ()=> getPostCountByCategory(2));
    const pageArr = [];
    //현재 페이지
    ...
    const bugPageNum = Math.floor(bugPostNum / 20) + 1;
    if(isPostNumLoading === false){
        //탈출 조건: 최대 페이지 숫자 끝까지 가거나 or 숫자 10개 배열에 넣던가.
        for(let i= startPageNumber * 10; (i< (startPageNumber * 10) + bugPageNum) && i < startPageNumber * 10 + 10; i++){
            pageArr.push(i+1);
        }
    }
    //10이하, 
    return(
        <PageNumContainer>
            <Link to={"/board/bugReport/1"}><MoveIcon src={doubleLeft} /></Link>
            <Link to={`/board/bugReport/${(page === 1) ? 1 : page-1}`}><MoveIcon src={left} /></Link>
            ...
            {isPostNumLoading ? (<NumberSelected>1</NumberSelected>) : (
                pageArr.map( num =>
                    <>
                        ...
                        (<NumberPage>
                            <Link to={`/board/bugReport/${num}`}>{num}</Link>
                        </NumberPage>)}
                    </>
                )
            )}
            <Link to={`/board/bugReport/${(page === bugPageNum) ? bugPageNum : page+1}`}><MoveIcon src={right} /></Link>
            <Link to={`/board/bugReport/${bugPageNum}`}><MoveIcon src={doubleRight} /></Link>
        </PageNumContainer>
    );
}
